  ...
  return (
    <div className="flex flex-col h-full justify-between gap-6 select-none">
      ...
      <div className="space-y-4">
        <div>
          {!isCollapsed && (
            <p className="px-3 text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-2 transition-opacity duration-200">
              Embudo de Pacientes
            </p>
          )}
                    ...
                    <div className={`flex items-center ${isCollapsed ? 'justify-center' : 'gap-3'}`}>
                      ...
                      {!isCollapsed && (
                        <div className="flex flex-col text-left truncate">
                          ...
                          <span className={`text-[10px] leading-tight truncate ${isActive ? 'text-slate-300 dark:text-slate-300' : 'text-slate-400 dark:text-slate-500'}`}>
                            {item.description}
                          </span>
                        </div>
                      )}
      ...
      <div className="mt-auto pt-4 border-t border-slate-200/80 dark:border-slate-800">
        {!isCollapsed ? (
          <div className="p-3.5 bg-gradient-to-br from-slate-50 to-slate-100/60 dark:from-slate-900 dark:to-slate-800/60 rounded-xl border border-slate-200/70 dark:border-slate-800 shadow-sm space-y-2.5 transition-all">
          ...
          <div 
            ...
            className="p-2.5 bg-slate-50 dark:bg-slate-800/80 rounded-xl border border-slate-200/70 dark:border-slate-800 flex items-center justify-center relative cursor-pointer group"
          >
