  ...
  const handleGenerateCanvaAndDispatch = async (autoDispatch = false) => {
    ...
    try {
      ...
      toast({
        ...
        description: 'No se pudo conectar con Canva API. Si expiró la sesión, haz clic en "Conectar Canva".',
        variant: 'destructive',
      });
    } finally {
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      ...
      >
        <DialogContent className="max-w-4xl max-h-[96vh] p-5 sm:p-6 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl flex flex-col items-center z-[150] overflow-hidden">
          {/* Header en Modo Claro con botones alineados */}
          <DialogHeader className="w-full flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-slate-200 dark:border-slate-800">
            <div className="flex items-center gap-2.5">
              ...
              <DialogTitle className="text-sm font-bold text-slate-900 dark:text-white leading-tight">
            ...
            <div className="flex items-center gap-2 mr-6 flex-wrap">
              {/* Controles de Zoom */}
              <div className="flex items-center bg-slate-100 dark:bg-slate-800 rounded-xl p-1 border border-slate-200 dark:border-slate-700 mr-1">
                <button
                  type="button"
                  onClick={() => setFlyerZoom((prev) => Math.max(0.75, Math.round((prev - 0.25) * 100) / 100))}
                  disabled={flyerZoom <= 0.75}
                  className="p-1 rounded-lg text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-white dark:hover:bg-slate-700 disabled:opacity-40 transition-all cursor-pointer"
                  title="Reducir zoom"
                >
                ...
                <button
                  type="button"
                  onClick={() => setFlyerZoom(1)}
                  className="px-2 text-[11px] font-mono font-bold text-slate-700 dark:text-slate-200 hover:text-teal-600 transition-colors cursor-pointer"
                ...
                <button
                  type="button"
                  onClick={() => setFlyerZoom((prev) => Math.min(2.5, Math.round((prev + 0.25) * 100) / 100))}
                  disabled={flyerZoom >= 2.5}
                  className="p-1 rounded-lg text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-white dark:hover:bg-slate-700 disabled:opacity-40 transition-all cursor-pointer"
                  title="Aumentar zoom"
                >
                ...
                {flyerZoom !== 1 && (
                  <button
                    type="button"
                    onClick={() => setFlyerZoom(1)}
                    className="p-1 rounded-lg text-slate-400 hover:text-slate-700 dark:hover:text-white transition-colors cursor-pointer ml-0.5"
              ...
              {/* Botón Descargar PNG centrado */}
              <a
                href={canvaResult?.downloadPngUrl || canvaResult?.previewUrl}
                download={`Flyer_NexoSalud_${lead?.nombres || 'Oferta'}.png`}
                className="h-8.5 px-3.5 rounded-xl bg-purple-600 hover:bg-purple-700 text-white text-xs font-semibold flex items-center justify-center gap-1.5 transition-all shadow-sm shadow-purple-600/20 cursor-pointer"
              >
                <Download className="w-3.5 h-3.5 shrink-0" />
                <span className="leading-none">Descargar PNG</span>
              </a>

              {/* Botón Abrir en Canva centrado con icono de Canva */}
              {canvaResult?.designUrl && canvaResult.designUrl !== 'https://www.canva.com/' && (
                <a
                  href={canvaResult.designUrl}
                  target="_blank"
                  rel="noreferrer"
                  className="h-8.5 px-3.5 rounded-xl bg-slate-50 dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700 text-xs font-semibold flex items-center justify-center gap-2 transition-all shadow-2xs"
                >
          ...
          {/* Contenedor del Flyer con Zoom y Modo Claro */}
          <div className="w-full flex-1 min-h-[50vh] max-h-[75vh] flex items-center justify-center p-4 bg-slate-100/90 dark:bg-slate-950/60 border border-slate-200/90 dark:border-slate-800 rounded-2xl mt-3 overflow-auto">
            <div 
            ...
            >
              <img
                ...
                onClick={() => setFlyerZoom((prev) => (prev > 1 ? 1 : 1.75))}
                className={`max-h-[68vh] w-auto object-contain rounded-xl shadow-xl transition-all select-none ${
                  flyerZoom > 1 ? 'cursor-zoom-out' : 'cursor-zoom-in'
                }`}
                title="Haz clic para alternar zoom (o usa los controles arriba)"
              />
            </div>
          </div>
        </DialogContent>
