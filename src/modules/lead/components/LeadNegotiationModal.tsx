    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      ...
      >
        <DialogContent className="max-w-4xl max-h-[96vh] p-5 sm:p-6 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl flex flex-col items-center z-[150] overflow-hidden">
          {/* Header en Modo Claro con botones limpios, ergonómicos y bien proporcionados */}
          <DialogHeader className="w-full flex flex-row items-center justify-between gap-4 pb-3.5 border-b border-slate-200 dark:border-slate-800">
            <div className="flex items-center gap-3 min-w-0 pr-2">
              <div className="w-10 h-10 rounded-xl bg-purple-50 dark:bg-purple-950/60 border border-purple-200/80 dark:border-purple-800 flex items-center justify-center shrink-0 shadow-2xs">
                <CanvaIcon className="w-5 h-5 text-[#00C4CC]" />
              </div>
            ...
            <div className="flex items-center gap-2.5 mr-6 shrink-0 flex-nowrap">
              ...
              <a
                href={canvaResult?.downloadPngUrl || canvaResult?.previewUrl}
                download={`Flyer_NexoSalud_${lead?.nombres || 'Oferta'}.png`}
                className="h-10 px-4 rounded-xl bg-purple-600 hover:bg-purple-700 active:scale-[0.98] text-white text-xs sm:text-sm font-semibold inline-flex items-center justify-center gap-2 transition-all shadow-sm shadow-purple-600/20 whitespace-nowrap cursor-pointer"
              >
              ...
              {canvaResult?.designUrl && canvaResult.designUrl !== 'https://www.canva.com/' && (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="h-10 px-4 rounded-xl bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700/80 active:scale-[0.98] text-slate-800 dark:text-slate-100 border border-slate-200 dark:border-slate-700 text-xs sm:text-sm font-semibold inline-flex items-center justify-center gap-2 transition-all shadow-2xs whitespace-nowrap cursor-pointer"
                >
