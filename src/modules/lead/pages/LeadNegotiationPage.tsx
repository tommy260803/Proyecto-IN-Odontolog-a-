...
export default function LeadNegotiationPage() {
  ...
  const handleGenerateCanvaAndDispatch = async (sendEmailOption = false) => {
    ...
    try {
      const activeService = options.servicios?.find((s: any) => s.id_servicio.toString() === selectedServicioId);
      const activeSede = options.sedes?.find((s: any) => s.id_sede.toString() === selectedSedeId);
      ...
      const offerConditions = selectedOptData?.condiciones || condiciones || 'Promoción exclusiva con garantía clínica NexoSalud';

      const res = await leadService.negotiateAndDispatch(id, {
