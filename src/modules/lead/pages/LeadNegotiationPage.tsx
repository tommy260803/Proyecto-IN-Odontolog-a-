...
export default function LeadNegotiationPage() {
  ...
  const handleDispatchCanvaEmail = async () => {
    ...
    try {
      const selectedBranchObj = options?.sedes?.find((s: any) => s.id_sede?.toString() === selectedSedeId);
      const branchName = selectedBranchObj?.nombre || (lead?.zona ? `Sede ${lead.zona}` : 'Sede Principal NexoSalud');
      
      const reqServicioName = options?.servicios?.find((s: any) => s.id_servicio?.toString() === selectedServicioId)?.nombre
        || lead?.Solicitudes?.[0]?.Servicio?.nombre
        || 'Consulta Odontológica Especializada';

      const vigHours = selectedVigencia === '24h' ? 24 : selectedVigencia === '48h' ? 48 : selectedVigencia === '72h' ? 72 : 168;
      ...
      const res = await leadService.negotiateAndDispatch({
        ...
        preferredSchedule: selectedFranja,
        category,
